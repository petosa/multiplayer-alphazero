...
class TicTacToe(Game):
    ...
    def get_initial_state(self):
        env = 0
        return { "env":env, "obs":np.zeros((3, 3, 2 + 1), dtype=np.float32) } # Final plane is a turn indicator

    # Returns a 3x3 boolean ndarray indicating open squares. 
    def get_available_actions(self, s):
        return s["obs"][:, :, :2].sum(axis=-1) == 0
        
    # Place an X or O in state.
    def take_action(self, s, a):
        p = self.get_player(s)
        s = copy.deepcopy(s)
        s["obs"][:,:,p] += a.astype(np.float32) # Next move
        s["obs"][:,:,2] = (s["obs"][:,:,2] + 1) % 2 # Toggle player
        s["env"] += 1
        return s
